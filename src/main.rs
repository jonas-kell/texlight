...
use texlight_language_server::semantic_analyze::{analyze_program, IdentType, Semantic};
use texlight_language_server::semantic_token::LEGEND_TYPE;
use texlight_language_server::span::Span;
        ...
        let inlay_hint_list = hashmap
            ...
            .map(|(k, v)| {
                (
                    ...
                    k.end,
                    match v {
                        texlight_language_server::nrs_lang::Value::Null => "null".to_string(),
                        texlight_language_server::nrs_lang::Value::Bool(_) => "bool".to_string(),
                        texlight_language_server::nrs_lang::Value::Num(_) => "number".to_string(),
                        texlight_language_server::nrs_lang::Value::Str(_) => "string".to_string(),
                    },
                )
            })
            .filter_map(|item| {
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        let completions = || -> Option<Vec<CompletionItem>> {
            ...
            for (_, item) in completions {
                match item {
                    texlight_language_server::completion::ImCompleteCompletionItem::Variable(
                        var,
                    ) => {
                    ...
                    texlight_language_server::completion::ImCompleteCompletionItem::Function(
                        name,
                        args,
                    ) => {
                        ret.push(CompletionItem {
